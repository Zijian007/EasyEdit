...
class ModelAndTokenizer:
    ...
    def __init__(
        ...
        if model is None:
            assert model_name is not None
            model = AutoModelForCausalLM.from_pretrained(model_name, device_map=device, torch_dtype=torch_dtype, cache_dir = cache_dir)
...
def generate_manually_with_probs(model, tokenizer, inp: dict, max_length, target_words = None):
    def get_ids_from_words(tokenizer, words_list):
        ids = []
        for word in words_list:
            # 检查word是否为数字
            if word.isdigit():
                print("The target word is a number")
                # 如果是数字,取第二个token的id
                id = tokenizer.encode(word, add_special_tokens=False)[1]
            else:
                # 如果不是数字,取第一个token的id
                id = tokenizer.encode(word, add_special_tokens=False)[0]
            ids.append([id])
        return ids
    if target_words is not None:
      target_ids = get_ids_from_words(tokenizer, target_words)
    #手动生成文本
    input_ids = inp["input_ids"]
    generated_ids = input_ids
    ...
    for _ in range(max_length + 1):
      ...
      next_token_logits = outputs.logits[:, -1, :]
      probs = nn.functional.softmax(next_token_logits, dim=-1)
      target_prob = probs[0][target_ids[0][0]].cpu().detach().item()
    ...
    new_text = tokenizer.batch_decode(ids)
    # print(f"ids:{ids}")
    return generated_text, new_text, target_probs, tokens
